...
Revision 12.0 - August 06, 2017 - support for LZ7DP 80 m CW transceiver
...
#define TX_RX (4)   // (2 sided 2 possition relay) - for Farhan minima +5V to Receive 0V to Transmit !!! (see Your schema and change if need)
...
#define DIG_KEYER (A1)      // digital straight key input
...
int_fast32_t rxif=12000000; // IF freq, will be summed with vfo freq - rx variable
int_fast32_t rxbfo=11998932;  //BFO generator 
int_fast32_t rxRIT=0;
int RITon=0;
int_fast32_t rx600hz=100;   // cw offset  100
long cal=130;
...
int BTNdecodeON = 0;   
int keyerON = 0;
int BTNinc = 1; // set number of default band ---> (for 7MHz = 3)
...
void checkCW(){
  keyerON = digitalRead(DIG_KEYER);
  if ((keyDown == 0) && (keyerON == LOW)){
    ...
    inTx = 1;
    keyDown = 1;
    digitalWrite(CW_KEY, HIGH); //start the side-tone
  }
  ...
  //if we have keyup for a longish time while in cw rx mode
...
// new
  ...
  si5351.set_freq((rx+rxif*100L) , SI5351_CLK1);
...
pinMode(DIG_KEYER,INPUT);
digitalWrite(DIG_KEYER,HIGH);
...
void sendFrequency(double frequency) { 
	rx=frequency;
    //VFO CLK1
//	si5351.set_freq(((rx+rxRIT+rxif)*100LL), SI5351_CLK1);
  si5351.set_freq((((rxif-rx)+rxRIT)*100LL), SI5351_CLK1);
